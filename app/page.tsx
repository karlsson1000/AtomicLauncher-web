...
export default function AtomicLauncher() {
  ...
  const getButtonClass = (os: string) => {
    return currentOS === os
      ? "bg-[#4572e3] text-white px-4 sm:px-6 py-2.5 sm:py-3 text-sm sm:text-base font-medium rounded flex items-center gap-2 cursor-pointer hover:ring-2 hover:ring-[#4572e3] hover:ring-offset-2 hover:ring-offset-neutral-900 transition-all w-full sm:w-auto justify-center"
      : "bg-white text-black px-4 sm:px-6 py-2.5 sm:py-3 text-sm sm:text-base font-medium rounded flex items-center gap-2 cursor-pointer hover:ring-2 hover:ring-[#4572e3] hover:ring-offset-2 hover:ring-offset-neutral-900 transition-all w-full sm:w-auto justify-center";
  };
  ...
  return (
    <div className="min-h-screen bg-neutral-900 flex flex-col items-center justify-center p-4 sm:p-8 relative">
      {/* Background Image */}
      ...
      <div className="text-center relative z-10 flex-1 flex flex-col items-center justify-center w-full">
        {/* Launcher Image */}
        <div className="mb-6 sm:mb-10 md:mb-14 pointer-events-none select-none w-full px-2 sm:px-4" style={{ perspective: "1200px" }}>
          <div style={{ transform: "rotateX(8deg) rotateZ(1deg)" }}>
            <Image
              ...
              height={1080}
              priority
              className="w-full max-w-[1000px] h-auto border border-white/10 rounded-lg shadow-2xl shadow-black/80 mx-auto"
              draggable={false}
            />
          </div>
        </div>
        ...
        {/* Install Buttons */}
        <div className="flex flex-col sm:flex-row gap-3 sm:gap-4 justify-center w-full max-w-md sm:max-w-none px-4">
          <button
          ...
          >
            ...
            <span className="flex-1 sm:flex-initial">{downloading === 'windows' ? 'Downloading...' : 'Windows'}</span>
          ...
          >
            ...
            <span className="flex-1 sm:flex-initial">{downloading === 'linux' ? 'Downloading...' : 'Linux'}</span>
            <svg className="w-4 h-4 flex-shrink-0" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
              <path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"></path>
              <polyline points="7 10 12 15 17 10"></polyline>
              <line x1="12" y1="15" x2="12" y2="3"></line>
          ...
          >
            ...
            <span className="flex-1 sm:flex-initial">{downloading === 'macos' ? 'Downloading...' : 'macOS'}</span>
            <svg className="w-4 h-4 flex-shrink-0" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
              <path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"></path>
              <polyline points="7 10 12 15 17 10"></polyline>
              <line x1="12" y1="15" x2="12" y2="3"></line>
        ...
        {/* Error Message */}
        {error && !loading && (
          <p className="text-red-400 text-xs sm:text-sm mt-3 sm:mt-4">
